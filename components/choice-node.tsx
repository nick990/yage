...
export const ChoiceNode = memo(({ data, isConnectable, id, selected }: NodeProps) => {
  ...
  return (
    <div
      ref={nodeRef}
      className={`shadow-lg rounded-md border ${selected ? "border-violet-400 ring-2 ring-violet-200" : "border-slate-200"} bg-slate-50`}
      style={{
        width: `${width}px`,
        height: `${height}px`,
        position: "relative",
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="ml-2">
            <div className="text-sm font-bold text-slate-800">{data.title}</div>
          </div>
        </div>
      ...
      <Handle
        ...
        className="w-6 h-6 bg-violet-500 border-2 border-white"
      />
